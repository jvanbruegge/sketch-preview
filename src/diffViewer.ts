...
export function diffViewer(): void {
    const prFiles = Array.from(
        ...
        .map(el => ({
            ...
            filePath: el!
                .querySelector('.file-info .link-gray-dark[title]')!
                .getAttribute('title')
        }))
        .map(({ rawUrl, filePath }) => {
            const repoUrl = rawUrl.substring(0, rawUrl.indexOf('/blob/'));
            return [rawUrl, `${repoUrl}/blob/master/${filePath}?raw=true`];
        });
...
function displayDiff(data: ImageData2): void {
    ...
    if (div) {
        div.innerHTML = `
            ...
            <img style="display: block;" src="data:image/png;base64,${fromByteArray(
                new Uint8Array(data.data2)
            )}"></img>
            <hr />
