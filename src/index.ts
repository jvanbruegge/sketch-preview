...
function checkForSketchFiles(): void {
    if (
        window.location.href.indexOf('/pull/') !== -1 &&
        !document.querySelector('.State.State--purple')
    ) {
        diffViewer();
    } else {
        ...
        const files = Array.from(
            document.querySelectorAll(
                '.repository-content a#raw-url[href$=".sketch"]'
            )
...
function displayZip(previewData: ImageData): void {
    ...
    if (div) {
        div.innerHTML = `<img src="data:image/png;base64,${fromByteArray(
            preview
        )}"></img>`;
